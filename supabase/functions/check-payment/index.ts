...
serve(async (req) => {
  ...
  try {
    ...
    // Check for successful payments in the last 90 days
    ...
    return new Response(JSON.stringify({ has_access: hasAccess, access_until: accessUntil }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 200,
    });
  } catch (error) {
